...
def reconstruct_model(params: dict) -> Pipeline:
    """
    Construye un Pipeline de scikit-learn instanciado con los mejores hiperparámetros.

    Parameters
    ----------
    params : dict
        Diccionario que contiene los hiperparámetros optimizados devueltos por Optuna.
    ...
    ------
    ValueError
        Si el nombre del clasificador en params no está soportado.
    """
...
def run_model_training():
    ...
    ------
    FileNotFoundError
        Si no se encuentran los parámetros guardados o los datos de entrenamiento.
    Exception
        Si ocurre cualquier error durante el proceso de entrenamiento o guardado.
    """
    ...
    if not os.path.exists(BEST_PARAMS_FILE):
        ...
        raise FileNotFoundError("best_params.pkl is missing.")
    ...
    try:
        ...
        if not os.path.exists(X_train_path) or not os.path.exists(y_train_path):
            raise FileNotFoundError("No se encontraron los archivos procesados X_train.csv o y_train.csv.")
